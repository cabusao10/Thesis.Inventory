...
{
    public class GetOrderById : BasePageRequest,IRequest<Result<BasePageReponse<GetAllOrdersResponse>>>
    {
        public GetOrderById(int orderid ,int page, int count)
        {
        ...
        public class Handler : BaseService, IRequestHandler<GetOrderById, Result<BasePageReponse<GetAllOrdersResponse>>>
        {
            ...
            /// <inheritdoc/>
            public async Task<Result<BasePageReponse<GetAllOrdersResponse>>> Handle(GetOrderById query, CancellationToken cancellationToken)
            {
                var orders = await this.ThesisUnitOfWork.Orders.Entities.Where(x=> x.Id == query.OrderId).ToListAsync();
