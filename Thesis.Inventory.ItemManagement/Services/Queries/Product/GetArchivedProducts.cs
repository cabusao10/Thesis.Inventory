...
{
    public class GetArchivedProducts: BasePageRequest, IRequest<Result<BasePageReponse<GetProductResponse>>>
    {
        public GetArchivedProducts(int page, int count)
        {
            this.Page = page;
            this.Count = count;
        }
        ...
        /// </summary>
        public class Handler : BaseService, IRequestHandler<GetArchivedProducts, Result<BasePageReponse<GetProductResponse>>>
        {
            ...
            /// <inheritdoc/>
            public async Task<Result<BasePageReponse<GetProductResponse>>> Handle(GetArchivedProducts query, CancellationToken cancellationToken)
            {
                var products = this.ThesisUnitOfWork.Products.Entities.Where(x => x.Status == Shared.Enums.ProductStatusType.Archived).GetPaged(query.Page, 100).ToList();
