...
{
    public class MobileLogin : IRequest<Result<UserLoginResponse>>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="LoginUserCommand"/> class.
        /// </summary>
        /// <param name="request">Login request.</param>
        public MobileLogin(UserLoginRequest request)
        {
        ...
        /// Handler for command.
        /// </summary>
        public class Handler : BaseService, IRequestHandler<MobileLogin, Result<UserLoginResponse>>
        {
            ...
            /// <inheritdoc/>
            public async Task<Result<UserLoginResponse>> Handle(MobileLogin command, CancellationToken cancellationToken)
            {
                try
                {
                    ...
                    var user = await this.ThesisUnitOfWork.Users.Entities
                        .Where(x=> x.Role == Shared.Enums.UserRoleType.Consumer)
                        .FirstOrDefaultAsync(x => x.Username == request.Username && x.Password == encrpyted_password);
